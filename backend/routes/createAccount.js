...
router.post("/", authenticateToken, async (req, res) => {
  ...
  db.get("SELECT * FROM Server WHERE id = ?", [serverId], async (err, server) => {
    ...
    // NEW: Check active accounts limit instead of total_create_akun
    db.get(`
    ...
    `, [serverId], async (countErr, countResult) => {
      ...
      if (activeAccounts >= server.batas_create_akun) {
        console.log(`[CreateAccount] Server ${serverId} has reached maximum active account limit: ${activeAccounts}/${server.batas_create_akun}`);
        return res.status(400).json({ 
          success: false, 
      ...
      console.log(`[CreateAccount] Server ${serverId} capacity check passed: ${activeAccounts}/${server.batas_create_akun} active accounts`);
