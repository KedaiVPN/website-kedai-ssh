...
router.post('/servers', async (req, res) => {
  const {
    ...
    status = 'online', batas_create_akun = 1000, url_monitoring, ip_server, member_1ip = 330, member_2ip = 430,
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO Server (domain, auth, nama_server, location, protocols, status, batas_create_akun, url_monitoring, ip_server) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [domain, auth, nama_server, location, protocols, status, batas_create_akun, url_monitoring || null, ip_server || null]
    );
...
router.put('/servers/:id', async (req, res) => {
  const {
    domain, auth, nama_server, location, protocols, status, batas_create_akun, url_monitoring, ip_server,
  ...
  try {
    ...
    await connection.query(
      `UPDATE Server SET domain = ?, auth = ?, nama_server = ?, location = ?, protocols = ?, status = ?, batas_create_akun = ?, url_monitoring = ?, ip_server = ? WHERE id = ?`,
      [domain, auth, nama_server, location, protocols, status, batas_create_akun, url_monitoring || null, ip_server || null, req.params.id]
    );
